...
CREATE TABLE batch_schema.flyway_schema_history (
...
CREATE TABLE batch_schema.BATCH_JOB_EXECUTION  (
     ...
     constraint JOB_INST_EXEC_FK foreign key (JOB_INSTANCE_ID)
         references batch_schema.BATCH_JOB_INSTANCE(JOB_INSTANCE_ID)
) ;
...
CREATE TABLE batch_schema.BATCH_JOB_EXECUTION_PARAMS  (
    ...
    IDENTIFYING CHAR(1) NOT NULL,
            constraint JOB_EXEC_PARAMS_FK foreign key (JOB_EXECUTION_ID)
                references batch_schema.BATCH_JOB_EXECUTION(JOB_EXECUTION_ID)
) ;
...
CREATE TABLE batch_schema.BATCH_STEP_EXECUTION_CONTEXT  (
              ...
              constraint STEP_EXEC_CTX_FK foreign key (STEP_EXECUTION_ID)
                  references batch_schema.BATCH_STEP_EXECUTION(STEP_EXECUTION_ID)
) ;
